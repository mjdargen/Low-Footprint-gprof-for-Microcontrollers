...
void LCD_GPIO_Init(void) {
	...
	// mjdargen
	(void)LCD_Data_Bits_Init(NULL);
	(void)LCD_D_NC_Init(NULL);
	(void)LCD_NWR_Init(NULL);
	(void)LCD_NRD_Init(NULL);
	(void)LCD_NRST_Init(NULL);
}

/* Initialize hardware for LCD backlight control and set to default value. */
static void LCD_Init_Backlight(void) {
	// TPM1 already initialized by PE
	(void)LCD_BL_PWM_Init(NULL);
}
...
void LCD_Set_Backlight_Brightness(uint32_t brightness_percent){
	...
	LCD_BL_PWM_SetDutyMS(NULL, (brightness_percent*LCD_BL_PERIOD)/100);
}
...
{
	LCD_GPIO_Init();
	// Already initialized LCD_TS_Init();
	LCD_Init_Backlight();
