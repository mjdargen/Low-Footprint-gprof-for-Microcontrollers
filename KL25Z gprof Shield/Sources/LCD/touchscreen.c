...
uint32_t LCD_TS_X_Scale=209;
uint32_t LCD_TS_X_Offset=6648;
uint32_t LCD_TS_Y_Scale=159;
uint32_t LCD_TS_Y_Offset=6689;
...
uint32_t LCD_TS_Read(PT_T * position) {
	...
	vTaskDelay(pdMS_TO_TICKS(TS_DELAY));
	...
	if (b>0) {
		...
		// Read inputs
		AD1_MeasureChan(TRUE, AdcLdd1_LCD_TS_YU);
		AD1_GetChanValue16(AdcLdd1_LCD_TS_YU, &yu);
		x = yu;
		...
		// Read inputs
		AD1_MeasureChan(TRUE, AdcLdd1_LCD_TS_XL);
		AD1_GetChanValue16(AdcLdd1_LCD_TS_XL, &xl);
		y = xl;
